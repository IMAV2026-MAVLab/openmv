...
void imlib_find_rects(list_t *out, image_t *ptr, rectangle_t *roi, uint32_t threshold) {
    ...
    int *theta_buffer = uma_malloc(sizeof(int) * r_diag_len, UMA_DTCM);
    uint32_t *mag_buffer = uma_malloc(sizeof(uint32_t) * r_diag_len, UMA_DTCM);
    point_t *point_buffer = uma_malloc(sizeof(point_t) * r_diag_len, UMA_DTCM);
